...
import { generateThumbnailBuffer, stripExifBuffer } from '../services/thumbnail.service'
  ...
  setImmediate(async () => {
    try {
      ...
      if (tipo === 'imagens') {
        ...
        const thumbnailUrl = await uploadToR2(thumbKey, thumbBuffer, 'image/webp')
...
export const getMidiaByIdHandler = asyncHandler(async (req: Request, res: Response) => {
  const midia = await getMidiaById(req.params.id)
  if (!midia) throw new AppError('Mídia não encontrada.', 404)

  res.json({ success: true, data: { ...midia, tamanhoBytes: midia.tamanhoBytes.toString() } })
})

// ─── DELETE /api/midias/:id ───────────────────────────────────────────────────

export const deleteMidiaHandler = asyncHandler(async (req: Request, res: Response) => {
  const midia = await getMidiaById(req.params.id)
  if (!midia) throw new AppError('Mídia não encontrada.', 404)
