          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <span className="text-white/60 text-sm">
                ...
                {pendingFilesState.length === 1 ? "arquivo pendente" : "arquivos pendentes"}
              </span>
                      ...
                      {filesOfTipo.map((file) => (
                        <div key={file.id} className="py-4 flex items-start space-x-4">
                          <div className="flex-shrink-0 w-16 h-16 flex items-center justify-center rounded-lg bg-cafta-primary/50">
                            {(() => {
                              ...
                              if (["jpg", "jpeg", "png", "gif", "webp"].includes(extension)) {
                                return <img src={file.url} alt={file.titulo} className="w-12 h-12 object-cover rounded" />;
                              }
                              if (["mp4", "webm", "ogg"].includes(extension)) {
                                return <video src={file.url} className="w-12 h-12 object-cover rounded" muted loop />;
                              }
                              return (
                          ...
                          <div className="flex-1 space-y-2">
                            <div className="flex items-center justify-between">
                              <h3 className="text-white font-semibold truncate max-w-[200px]">{file.titulo}</h3>
                              <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${tipoColors[tipo] || "bg-gray-500/50"}`}>
                                {tipoLabels[tipo] || tipo}
                              </span>
                            </div>

                            <p className="text-white/40 text-sm">{file.dataUpload}</p>
