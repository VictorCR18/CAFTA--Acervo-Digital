...
import { ArquivoAcervo } from "./../src/types/index";
import { api } from "@/lib/api";

interface UseAcervoItemsOptions {
  searchTerm?: string;
}

export function useAcervoItems(options: UseAcervoItemsOptions = {}) {
  ...
  useEffect(() => {
    async function fetchAcervoItems() {
      ...
      try {
        // Fetch from backend API with optional search
        const response = await api.get(
          `/api/midias${searchTerm ? `?search=${encodeURIComponent(searchTerm)}` : ""}`,
        );
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Erro ao carregar itens do acervo",
        );
      } finally {
        setLoading(false);
      }
